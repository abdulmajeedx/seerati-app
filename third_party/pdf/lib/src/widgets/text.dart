...
abstract class _Span {
  ...
  /// Pen advance; what the next word is laid out from.
  double get advance => width;
...
class _Word extends _Span {
  ...
  @override
  double get advance => metrics.advanceWidth;
...
class _Line {
  ...
  void realign(double totalWidth) {
      ...
      case TextAlign.justify:
        ...
        for (final span in spans) {
          span.offset = PdfPoint(
            isRTL
                ? delta - x - (span.offset.x + span.advance)
                : span.offset.x + x,
    ...
    if (isRTL) {
      for (final span in spans) {
        span.offset = PdfPoint(
          delta - (span.offset.x + span.advance),
          span.offset.y - baseline,
        );
      }
      return;
